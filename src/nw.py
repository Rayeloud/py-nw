...
if __name__ == '__main__':
    ...
    with open(f"{PATH}param.log", "w") as f:
        print(f'--------------------------------------------------', file=f)
    ...
    if RESTART:
        ...
        if not SPINODAL:
            ...
            torch.manual_seed(seed)
            R = (12*0.5)/dX
            xx, yy, zz = torch.meshgrid(torch.arange(nx), torch.arange(ny), torch.arange(nz), indexing='ij')
            # 0 outside 1 inside circle
            c = (((xx-nx//2)**2 + (yy-ny//2)**2 < R**2).float())
            ...
            if SUBSTRATE:
                indic = makeIndicatorFunction(Lx, Ly, Lz, dx, dy, dz, (R*3/4)*dX, type='planar')
                #indic = makeIndicatorFunction(Lx, Ly, Lz, dx, dy, dz, 400, type='circular')
                c = (1-indic) * c
            ...
            print(f'--------------------------------------------------')
            theta = 90
            FILENAME = f"{FILENAME}_spinodal_test"
            theta = theta * np.pi/180
            saveFrame(c, t_start, save_every, config, dt, path=PATH, filename=FILENAME)
    ...
    with torch.no_grad():
        ...
        filter_width = 3 * max(np.abs(_kx[1] - _kx[0]), np.abs(_ky[1] - _ky[0]), np.abs(_kz[1] - _kz[0])) if len(_kz) > 1 else 3 * max(np.abs(_kx[1] - _kx[0]), np.abs(_ky[1] - _ky[0]))
        ...
        jkx = jkx.to(device=device)
        jky = jky.to(device=device)
        jkz = jkz.to(device=device)
        ...
        grad_indic1_device.copy_(torch.fft.ifftn(jkx*indic_device_fft, dim=(0, 1, 2))) if SUBSTRATE else None
        grad_indic2_device.copy_(torch.fft.ifftn(jky*indic_device_fft, dim=(0, 1, 2))) if SUBSTRATE else None
        grad_indic3_device.copy_(torch.fft.ifftn(jkz*indic_device_fft, dim=(0, 1, 2))) if SUBSTRATE else None
        ...
        for t in tqdm(t_range, desc="Solving..."):
            ...
            if not SUBSTRATE:
                ...
                compute_mu_fft_bis(mu_device_fft, c_device, c_device_fft, flux_c_device_fft, bc_device_fft, g_device_fft,
                                   (grad_indic1_device, grad_indic2_device, grad_indic3_device), grad_indic_norm_device, low_pass_filter,
                                   kappa, jkx, jky, jkz, k2, theta)
                # compute_fftn(36*A*c_device*c_device*(1-c_device-indic_device)*indic_device, out=source_term_device_fft, filter=low_pass_filter)
                # compute_fftn(4*A*c_device*(1-c_device-indic_device)*indic_device, out=source_term_device_fft, filter=low_pass_filter)
                # mu_device_fft.add_(-source_term_device_fft)

            # compute grad_mu_hat (F(grad(mu)) = jk * mu_hat )
            compute_grad_mu((grad_mu1_device, grad_mu2_device, grad_mu3_device), mu_device_fft, jkx, jky, jkz)
            ...
            compute_euler_timestep(c_device_fft, flux1_device_fft, flux2_device_fft, flux3_device_fft, jkx, jky, jkz, k4, kappa, alpha, dt)
